...
import { verifyPasswordResetOtp, resendPasswordResetOtp } from "../../../lib/api";

// Helper to get email from router state or query
function useEmailFromRoute(): string | null {
...
export const OtpVerify: React.FC = () => {
    ...
    const handleVerify = async () => {
        ...
        try {
            ...
            if (ok?.data) {
                ...
                const id = ok?.data?.publicId;
                // Navigate to change password or next step. Adjust as needed.
                navigate("/change/password", { state: { ref: id, email: email } });
            } else {
            ...
            setError(e?.message || "Unable to verify code. Try again.");
            show({ type: "error", title: "Verification failed", message: e?.message || "Network error" });
        } finally {
            setSubmitting(false);
        }
    };
    ...
    const handleResend = async () => {
        ...
        try {
            ...
            show({ type: "success", title: "Code sent", message: `A new code was sent to ${email}` });
            setCooldown(30); // 30s cooldown
        } catch (e: any) {
            show({ type: "error", title: "Cannot resend", message: e?.message || "Please try again later" });
        } finally {
            setResending(false);
        }
    };
                ...
                <div className="space-y-6">
                    ...
                    <div className="flex items-center justify-between text-sm">
                        <button
                            ...
                            className={cn(
                                "font-bold text-primary-500 hover:text-primary-600 transition-colors",
